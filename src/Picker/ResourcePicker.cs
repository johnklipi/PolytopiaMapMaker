...
internal class ResourcePicker  : PickerBase
{
    ...
    {
       return Utils.GetSprite((int)Brush.chosenResource, SpriteData.ResourceToString(Brush.chosenResource), gameLogicData);
    }
    ...
    internal override void CreatePopupButtons(ref float num, SelectViewmodePopup selectViewmodePopup, GameState gameState)
    {
        foreach (ResourceData resourceData in gameState.GameLogicData.AllResourceData.Values)
        {
            ...
            Utils.CreateChoiceButton(selectViewmodePopup, resourceName,
            ...
            {
                Utils.SetIcon(button, Utils.GetSprite(type, SpriteData.ResourceToString((ResourceData.Type)type), gameState.GameLogicData), 0.6f);
            }
        }
    }
}